...
def generate_telnet_ports(start,stop,folder):
    result = []
    for i in range(start, stop + 1):
        list_result = ['port_' + str(i),'127.0.0.1', folder, 'telnet', i, 'xterm']
        result.append(list_result)
    return result
telnet_list = generate_telnet_ports(start=args.start,stop=args.stop,folder=args.folder)
'''write header'''
